...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!shippingAddress.name || !shippingAddress.email || !shippingAddress.address || !shippingAddress.city || !shippingAddress.pincode) {
      toast({
        title: "Error",
    ...
    try {
      ...
      // Build shipping address object with correct field names
      const shippingAddressData = {
        ...
        postalCode: shippingAddress.pincode // Map pincode to postalCode for consistency
      };
      ...
      const orderData = {
        ...
        status: 'pending',
        user_id: null // Anonymous order - order_number will be auto-generated by database trigger
      };
      ...
      const { data: order, error } = await supabase
        .from('orders')
        .insert(orderData as any) // Cast to avoid type error since order_number will be auto-generated
        .select()
        .single();

      if (error) {
        console.error('Order insertion error details:', {
      ...
      toast({
        ...
        description: `Your order #${order.order_number} has been confirmed. You will receive an email notification shortly.`
      });
      ...
      if (error.message?.includes('violates row-level security')) {
        errorMessage = "Order access denied. Please ensure all required information is provided.";
      } else if (error.message?.includes('not null violation')) {
        errorMessage = "Required order information is missing. Please check all fields.";
      } else if (error.code === 'PGRST116') {
        errorMessage = "Order validation failed. Please verify your information.";
