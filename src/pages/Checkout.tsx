...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Generate a temporary order number since trigger might not be working
      const tempOrderNumber = 'MNG-ORD-' + Math.random().toString(36).substr(2, 9).toUpperCase();
      
      // Insert order into database
      const { data, error } = await supabase
        .from('orders')
        .insert({
          user_id: user?.id || null, // Support anonymous orders
      ...
      toast({
        ...
        description: `Your order #${data.order_number} has been confirmed.`
      });
