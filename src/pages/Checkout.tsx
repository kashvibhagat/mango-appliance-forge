...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // For COD, insert order directly and complete
      ...
      // For online payment (UPI/Card), create Razorpay order
      ...
      const { data: razorpayData, error: razorpayError } = await supabase.functions.invoke(
        'create-razorpay-order',
        {
          body: {
            amount: finalTotal,
            currency: 'INR',
            receipt: orderDbData.order_number,
            notes: {
      ...
      await supabase
        .from('orders')
        .update({ razorpay_order_id: razorpayData.order_id })
        .eq('id', orderDbData.id);

      // Open Razorpay checkout
      const options = {
        key: 'rzp_live_RgkZERfcxAdAjZ',
        ...
        handler: async function (response: any) {
          try {
            ...
            toast({
              ...
              description: `Your order #${orderDbData.order_number} has been confirmed.`
            });
            
            navigate(`/order-success?orderId=${orderDbData.order_number}`);
          } catch (error: any) {
        ...
        modal: {
          ondismiss: function() {
            ...
            toast({
              ...
              description: "Your order has been saved. You can retry payment from your orders page.",
              variant: "destructive"
            });
          }
        }
